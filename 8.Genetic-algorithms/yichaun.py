...
def fitnessFunction():
    # return lambda a, b: 0.5 - (np.sin(np.sqrt(a**2+b**2))**2 - 0.5)/(1 + 0.001*(a**2 + b**2)**2)
    # return lambda a, b: np.sin(a * b) - np.cos(a * b) + np.sin(a)
    # return lambda a, b: 21.5 + a*np.sin(np.pi*4*a) + b*np.sin(np.pi*20*b)
    # return lambda a, b: -a**2-b**2
    return lambda a, b: 3 * np.power((1 - a), 2) * np.exp(-a ** 2 - (b + 1) ** 2) -\
        10 * (a / 5 - np.power(a, 3) - np.power(b, 5)) * np.exp(-a ** 2 - b ** 2) - \
        np.exp(-(a + 1) ** 2 - b ** 2) / 3
...
def plot():
    ...
    # Z = np.sin(X*Y) - np.cos(X*Y) + np.sin(X)
    # Z = np.exp(-X**2-Y*2/2)*np.cos(4*X)+np.exp(-3*((X+0.5)**2+Y**2/2))
    # Z = -X**2-Y**2
    Z = 3*np.power((1-X), 2)*np.exp(-X**2-(Y+1)**2) - \
        10*(X/5-np.power(X, 3)-np.power(Y, 5))*np.exp(-X**2-Y**2)-\
        np.exp(-(X+1)**2-Y**2)/3
    plt.xlabel('x')
    plt.ylabel('y')
    ax.set_zlim([-10, 10])
