...
with open('data1/16data.txt') as f:
    # 忽略回应信息
    res = f.read()[21:-3]
    index = 0
    for i in range(int(len(res)/15)):
        response = res[index:index+15].replace(' ', '')  # 去除空格
        b = bin(int(response, 16))[2:]  # 转为二进制
        distance = int(b[-8:] + b[-16:-8], 2)
        theta = int(b[-24:-17] + b[-32:-24], 2)
        x.append(distance*np.sin(theta))
        y.append(distance*np.cos(theta))
        index += 15
    plt.scatter(x, y)
    plt.show()
...
orders,reach_dists = OPTICS(data, np.inf, 80)
end = time.clock()
print('finish all in %s' % str(end - start))
labels=extract_dbscan(data,orders,reach_dists, 500)
plotReachability(reach_dists[orders], 500)
